...
class Surrogate_Accelerator_v1( gym.Env ) :
    ...
    def step(self , action) :
        ...
        DENORN_BVIMIN = unscale( self.variables[0] , np.array( [self.VIMIN] ).reshape( 1 , -1 ) ,
                                 self.scale_dict )  # self.scalers[0].inverse_transform(np.array([self.VIMIN]).reshape(1, -1))
        ...
        B_VIMIN_trace = unscale( self.variables[2] , self.state[0 , 2 , :].reshape( -1 , 1 ) ,
                                 self.scale_dict )  # self.scalers[2].inverse_transform(self.state[0, 2, :].reshape(-1, 1))
        BIMINER_trace = unscale( self.variables[1] , self.state[0 , 1 , :].reshape( -1 , 1 ) ,
                                 self.scale_dict )  # self.scalers[1].inverse_transform(self.state[0, 1, :].reshape(-1, 1))
        ...
        self.data_state = np.copy(
            self.X_train[self.batch_id + self.steps].reshape( 1 , self.nvariables , self.nsamples ) )
        data_iminer = unscale( self.variables[1] ,
                               self.data_state[0][1][self.nsamples - 1].reshape( 1 , -1 ) ,
                               self.scale_dict )  # self.scalers[1].inverse_transform(self.data_state[0][1][self.nsamples - 1].reshape(1, -1))
    ...
    def render(self) :
        ...
        # BVIMIN_trace = unscale ( self.variables [ 0 ] , self.state [ 0 , 0 , 1 :-1 ].reshape ( -1 , 1 ) ,
        #                          self.scale_dict )  # self.scalers[0].inverse_transform(self.state[0, 0, 1:-1].reshape(-1, 1))
        # BIMINER_trace = unscale ( self.variables [ 1 ] , self.state [ 0 , 1 , : ].reshape ( -1 , 1 ) ,
        #                           self.scale_dict )  # self.scalers[1].inverse_transform(self.state[0, 1, :].reshape(-1, 1))
        B_VIMIN_trace = unscale( self.variables[2] , self.state[0 , 2 , :].reshape( -1 , 1 ) ,
        ...
        for v in range( 0 , nvars ) :
            ...
            if v == 0 :
                axs[v].set_title(
                    'Raw data reward: {:.2f} - RL agent reward: {:.2f} - PID Eq reward {:.2f}'.format(
            ...
            if v == 1 :
                x = np.linspace( 0 , LOOK_BACK - 1 ,
                                 LOOK_BACK )  # np.linspace(0, 149, 150) #TODO: change this so that it is dynamic for lookback
